...
namespace proekt_internetTeh.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult AddToFavorite(int id)
        {
            ...
            var dosegasniOmileni = db.Favorites.SingleOrDefault(e=>e.Email==model.Email);
            if(dosegasniOmileni!=null)
            {
                model.OmileniOglasi = dosegasniOmileni.OmileniOglasi.ToList();
                model.OmileniOglasi.Add(oglas);
                dosegasniOmileni.OmileniOglasi = model.OmileniOglasi;
            }
            else
            {
                model.OmileniOglasi.Add(oglas);
        ...
        public ActionResult DeleteFavOglas(int id)
        {
            var oglas = db.Oglas.Find(id);
            var user = UserManager.FindById(User.Identity.GetUserId());
            var dosegasniOmileni = db.Favorites.SingleOrDefault(e => e.Email == user.Email);
            dosegasniOmileni.OmileniOglasi.Remove(oglas);
            db.SaveChanges();
            return RedirectToAction("ViewFavorite");
        }
